(function () {
  ...
  var App = makeComponent($option => {
    ...
    function insertToParent() {
      $$apply();
      parentArray.push(id++);
      console.log("parentArray now- ", JSON.stringify(parentArray));
    }
    parentArray.push(101);
    window.parentArray = parentArray;
    console.log("hhh");
    {
      const $parentElement = htmlToFragment(`<button> Add to array inside parent </button><h3> </h3><br/><br/>`, 3);
      let [el0, el1] = refer($parentElement, '+1+');
      addEvent(el0, 'click', ($event) => { insertToParent($event);});
